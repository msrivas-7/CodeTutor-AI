...
export function renderMarkdown(markdown: string): string {
  ...
  while (index < lines.length) {
    ...
    if (fence) {
    ...
    } else if (line.includes("|") && isTableDelimiter(lines[index + 1])) {
      ...
      index -= 1;
      out.push(
        `<div class="table-scroll"><table><thead><tr>${headers.map((cell) => `<th scope="col">${inlineMarkdown(cell)}</th>`).join("")}</tr></thead><tbody>${rows.map((row) => `<tr>${headers.map((_, cellIndex) => `<td>${inlineMarkdown(row[cellIndex] ?? "")}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`,
      );
    } else if (/^###\s+/.test(line)) {
...
.prose pre code{display:block;max-width:100%;overflow-x:auto}.prose .table-scroll{max-width:100%;overflow-x:auto;margin:1.3rem 0;border:1px solid var(--line);border-radius:1rem}.prose table{width:100%;table-layout:fixed;border-collapse:collapse;background:rgba(15,23,42,.5)}.prose th,.prose td{padding:.75rem .85rem;border-bottom:1px solid var(--line);text-align:left;vertical-align:top;color:#d7deea;overflow-wrap:anywhere}.prose th:first-child,.prose td:first-child{width:30%}.prose th{background:rgba(56,189,248,.07);color:var(--ink);font-weight:650}.prose tbody tr:last-child td{border-bottom:0}.chip code{color:var(--ink);font:600 1em/1.3 "JetBrains Mono",ui-monospace,monospace}
#main:focus{outline:3px solid rgba(56,189,248,.62);outline-offset:-3px}
`;
...
export function renderLessonPage(course: DiscoveryCourse, lesson: DiscoveryLesson): string {
  ...
  const isFirstLesson =
    course.id === FIRST_LESSON_CONTRACT.courseId &&
    lesson.id === FIRST_LESSON_CONTRACT.lessonId;
  const trialLabel = isFirstLesson
    ? `Try lesson 1 — about ${FIRST_LESSON_CONTRACT.estimatedMinutes} minutes →`
