...
.prose pre code{display:block;max-width:100%;overflow-x:auto}
...
export function renderCategoryPage(catalog: DiscoveryCatalog): string {
  ...
  const consistentBody = body.replace(
    "Try a four-minute lesson →",
    `Try the first lesson — about ${FIRST_LESSON_CONTRACT.estimatedMinutes} minutes →`,
  );
  return documentShell({
    title,
    description,
