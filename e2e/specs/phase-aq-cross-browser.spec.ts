...
test.describe(
  ...
  test("Escape closes the product modal through a native viewport reflow", async ({ page }) => {
    ...
    await seedFirstRun(page, true);
    await openLesson(page);
    const returnFocus = page.getByRole("button", { name: /sign up to save/i });
    await returnFocus.click();
    const continuation = page.getByRole("dialog", { name: /sign up to save/i });
    await expect(continuation).toBeVisible();

    // Safari can apply this same physical Escape to its native fullscreen
    // transition. The product modal must still close and restore its trigger
    // after the resulting viewport reflow.
    await Promise.all([
      page.keyboard.press("Escape"),
