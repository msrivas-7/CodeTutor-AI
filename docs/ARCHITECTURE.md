...
- **Function-test harness** — capstone-style lessons declare a `function_tests` completion rule with hardcoded visible and hidden test cases (`{ name, call, expected, hidden?, category? }`). `POST /api/execute/tests` generates a per-run harness for the lesson's language that loads learner code (Python: `runpy.run_path` with `__main__`-guard bypass; JavaScript: `vm.createContext` with `require()` of `main.js`), then evaluates each test in an isolated scope using the language's literal parser on `expected` (Python: `ast.literal_eval`, JavaScript: `JSON.parse`). Results stream back as a sentinel-wrapped JSON payload (`__CODETUTOR_TESTS_v1_…__{…}__…__`) that the frontend parses into a `TestReport`. The instructions panel gains an Instructions ⇄ Examples tab when `function_tests` is present; Examples tab hosts the visible test cards plus a `FailedTestCallout` that surfaces one failure at a time (visible-first priority). Check My Work auto-switches to Examples on failure and shows a tab-header dot until viewed. Hidden test names/inputs are never leaked; the author-tagged `category` string appears only after two consecutive fails on the same hidden test.
- **Per-language harness layer** — the Python harness above is one implementation of a language-agnostic interface so additional languages can slot in without changing route code or the validator. `backend/src/services/execution/harness/types.ts` defines `HarnessBackend { language, generate(tests), parseOutput(stdout) }`. `registry.ts` is a `language → HarnessBackend` lookup; `runHarness.ts` is the single dispatch entry point. `/api/execute/tests` accepts `language: languageSchema`, dispatches through the registry, and returns a 422 when a language has no registered backend. Today `pythonHarness.ts` and `javascriptHarness.ts` are registered; additional languages (TS/Java/Go/Rust/Ruby) plug in here as real courses call for them. `content-lint` and the `verify-solutions` CLI consult the same registry so authors get an author-facing error if they declare `function_tests` on a language without backend support, and CI skips verification (rather than failing) with a loud `[skip: no harness for <lang>]` for declarative `expected_stdout`-only lessons that happen to be non-Python. `LANGUAGE_ENTRYPOINT: Record<Language, string>` in `frontend/src/types.ts` is the single source of truth for entry filenames (`python → main.py`, `javascript → main.js`, …); scripts mirror this via `frontend/scripts/language.ts` (`entryFileFor`, `fileExtForLanguage`, `SCAFFOLD_LANGUAGES`, per-language `functionStub`). Courses can flag themselves `"internal": true` in `course.json` — `courseLoader.listPublicCourses()` filters those out of learner-facing listings, while `ContentHealthPage` uses the unfiltered list. The `_internal-js-smoke` course exists to keep every non-Python code path exercised end-to-end: schema validates, loader reads `main.js` starter, `LessonPage` routes the run, `verify-solutions` dispatches correctly, `content-lint` stays clean.
- **Dev profile switcher (dev-only)** — everything under `frontend/src/__dev__/` is gated on `import.meta.env.DEV` and tree-shaken out of production bundles. A pre-hydration `bootstrap.ts` runs as the very first import in `main.tsx`, re-applying any active frozen profile's seed before the Zustand stores read localStorage. Ten canned profiles (nine frozen + one sandbox) exercise major UI states (fresh install, mid-course, shaky mastery, capstone-first-fail, all-complete, etc.). Aggregate-state profiles (`mid-course-healthy`, `needs-help-dashboard`, `capstones-pending`, `all-complete`) seed progress for *both* registered courses simultaneously so the multi-course dashboard, review card, and celebration flow all render against real polyglot state; narrative-specific profiles stay single-course because their story is tied to a specific lesson. switching is driven by a `Cmd/Ctrl+Shift+Alt+D` capture-phase shortcut (uses `event.code === "KeyD"` so Mac Option-transformed keys still match) and a Developer tab in `SettingsPanel`. An allow-list in `applyProfile.ts` ensures OpenAI keys, theme, and UI size prefs are never touched by a profile swap; a one-shot `__dev__:realSnapshot` captured on first enable guarantees an exit path back to the dev's real user state.
...
- **Content lint (`frontend/scripts/content-lint.ts`)** — the CLI that `npm run lint:content` drives. Parses every file against the Zod schemas, then layers on structural checks: folder id ↔ `lesson.json.id` ↔ `courseId` parent, unique + contiguous `order`, prereqs appear earlier in `lessonOrder`, practice-ids unique within a lesson, `starter/` non-empty, `content.md` non-empty, `function_tests.expected` round-trips through the target language's literal parser (Python: `ast.literal_eval` via subprocess; JavaScript: `JSON.parse`), `required_file_contains.file` (when present) names a real starter file, multi-file `starter/_index.json` shape matches the loader, and `function_tests` is blocked on lessons below each language's authoring floor (Python: order 6, JavaScript: order 5 — enforced by `FUNCTION_TESTS_ORDER_FLOOR_BY_LANGUAGE`). Runs in CI as the `content-lint` job.
- **Golden solutions + verify-solutions (`frontend/scripts/verify-solutions.ts`)** — every lesson and every practice exercise has a committed correct solution under `lessons/<id>/solution/`. The verifier copies each solution into a private temp dir and runs every `completionRule` against it with the same engine the production validator uses (per-language harness for `function_tests`, a language-appropriate subprocess — `python3` or `node` — for `expected_stdout`, a string check for `required_file_contains`). Runs in CI as the `solutions-pass` job, which `needs: content-lint`. Uses `python3` directly rather than the runner container — we trust our own solutions, and sandboxing is reserved for learner code.
...
- **End-to-end (`e2e/`, Playwright)** — drives the real product: Vite dev server + Dockerized backend + Python runner + real localStorage/IndexedDB. Catches UI-integration regressions that unit tests miss (Monaco focus, modal portals, SSE streaming, Zustand hydration, router navigation). Chromium-only, 101 specs across 14 files: editor / guided-learning / function-tests / dev-profiles / progress-I/O / AI-tutor (mocked) / onboarding / coach-rail / settings / lesson-edge / practice / js-smoke / js-course. OpenAI is mocked by default (`aiMocks.ts` replays canned SSE frames); an opt-in `specs/real-api/**` suite (`E2E_REAL_OPENAI=1`) hits a real key for release-gate smoke. Deterministic starting states come from the dev-profile seed JSONs, regenerated from `__dev__/profiles.ts` via `npm run dump-seeds` so fixtures can't drift. CI job: `.github/workflows/e2e.yml`.
