...
| Eval v2 automated gate | **Met for engineering release** | The complete 60-case dataset, zero-hidden-error gate, deterministic validators, per-intent floors, provenance fingerprints, and reviewed baseline are implemented and green. B3 also records an independent complete replay against the production routing policy. |
