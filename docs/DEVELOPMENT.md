...
- [E2E](../.github/workflows/e2e.yml) runs the complete Chromium inventory in an automatically selected duration-balanced topology of up to twelve shards with two workers each, an advisory two-shard critical shadow, and focused Firefox/WebKit journeys. The tracked [capacity record](../.github/e2e-shard-capacity.json) reserves all four support stacks beneath the proven sixteen-stack database ceiling, models measured setup cost, and falls back to the stable twelve-shard topology unless timing history is fresh and sufficiently complete. Suite growth changes the shard count automatically inside that envelope; capacity-boundary crossings recommend a controlled rebenchmark without ever authorizing test selection or weakening a gate.
